<x-app-layout>
    ...
    <div class="p-4 md:p-8 max-w-4xl mx-auto space-y-6">
        {{-- Page Header --}}
        <div>
            <h2 class="text-2xl font-bold text-[#171c1f]">Edit Pengumuman</h2>
            <p class="text-sm text-[#5c5f61] mt-0.5">Perbarui informasi pengumuman yang sudah ada</p>
        </div>

        {{-- Form Card --}}
        <div class="bg-white rounded-2xl shadow-soft border border-[#eaeef2] p-8">
            <form action="{{ route('pengumuman.update', $pengumuman) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select id="kategori" name="kategori"
                        ...
                        <option value="Administrasi" {{ old('kategori', $pengumuman->kategori) === 'Administrasi' ? 'selected' : '' }}>Administrasi</option>
                ...
                <div>
                    ...
                    <select id="prioritas" name="prioritas"
                        ...
                        <option value="Biasa" {{ old('prioritas', $pengumuman->prioritas) === 'Biasa' ? 'selected' : '' }}>Biasa</option>
                    </select>
                    @error('prioritas')
                        <p class="mt-1 text-xs text-[#ba1a1a]">{{ $message }}</p>
                    @enderror
                ...
                {{-- Meta Information --}}
                <div class="bg-[#f0fdf4] p-4 rounded-xl border border-[#0e7a3d]/20">
                    <p class="text-xs text-[#005f2d]">
                        <span class="font-semibold">Dibuat:</span> {{ $pengumuman->created_at->translatedFormat('d M Y H:i') }} |
                        <span class="font-semibold">Diperbarui:</span> {{ $pengumuman->updated_at->translatedFormat('d M Y H:i') }}
                    </p>
                </div>
