<x-app-layout>
    ...
    <div class="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
            ...
            <div class="lg:col-span-4 bg-white rounded-2xl p-6 shadow-soft border border-[#eaeef2] flex flex-col items-center text-center justify-between">
                <div class="flex flex-col items-center w-full">
                    ...
                    <p class="text-xs text-secondary mb-6">Siswa • NIS: {{ Auth::user()->nis ?? '-' }}</p>
                </div>
                <div class="w-full flex justify-between px-4 py-3 bg-[#f6fafe] rounded-xl border border-surface-container mt-auto">
                    <div class="flex flex-col items-center flex-1">
                        <span class="text-[9px] text-secondary font-bold uppercase tracking-wider">Peringkat</span>
                        <span class="text-primary font-bold text-sm mt-1">#4 / 32</span>
                    </div>
                    <div class="w-px bg-[#becabc] h-8 my-auto"></div>
                    <div class="flex flex-col items-center flex-1">
                        <span class="text-[9px] text-secondary font-bold uppercase tracking-wider">IPK</span>
                        <span class="text-primary font-bold text-sm mt-1">3.82</span>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-8 flex flex-col gap-6">
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    <div class="bg-primary-container text-on-primary-container rounded-2xl p-6 shadow-soft flex flex-col justify-center items-center text-center">
                        <span class="text-[10px] uppercase font-bold tracking-widest text-on-primary-container/80 mb-1">Presensi</span>
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-soft border-t-4 border-tertiary border-l border-r border-b border-surface-container flex flex-col justify-center items-center text-center">
                        <span class="text-[10px] uppercase font-bold tracking-widest text-secondary mb-1">Tugas Aktif</span>
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-soft border-t-4 border-primary border-l border-r border-b border-surface-container flex flex-col justify-center items-center text-center">
                        <span class="text-[10px] uppercase font-bold tracking-widest text-secondary mb-1">Materi Tersedia</span>
                        <div class="text-3xl font-bold text-primary">{{ $materiTersediaCount }}</div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl p-6 shadow-soft border border-surface-container flex-1 flex flex-col justify-between">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <span class="text-[10px] text-primary font-bold bg-[#f0fdf4] px-2 py-0.5 rounded-lg border border-primary-container/20">Semester Ini</span>
                    </div>
                    <div class="h-32 w-full flex items-end justify-between px-4 gap-4">
                        <div class="flex-1 bg-primary/20 rounded-t-lg h-[92%] relative group cursor-pointer hover:bg-primary/40 transition-all">
                        ...
                        <div class="flex-1 bg-primary rounded-t-lg h-[98%] relative group cursor-pointer hover:bg-primary/90 transition-all">
                        ...
                        <div class="flex-1 bg-primary/20 rounded-t-lg h-[95%] relative group cursor-pointer hover:bg-primary/40 transition-all">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    @forelse($tugasMendatang as $item)
                        <li class="flex items-center justify-between py-2 border-b border-[#f0f4f8] last:border-0">
                            <div>
                                ...
                                <p class="text-[11px] text-secondary mt-0.5">Deadline: {{ \Carbon\Carbon::parse($item->deadline)->translatedFormat('d M Y H:i') }}</p>
                            </div>
                            @php
                            ...
                            @else
                                <span class="px-2.5 py-1 text-[9px] font-bold bg-[#f0fdf4] text-primary rounded-full uppercase tracking-wider">{{ $diffInDays }} Hari</span>
                            @endif
                        </li>
                    @empty
                        <li class="py-12 text-center text-xs text-secondary">
            ...
            {{-- Scan Presensi --}}
            <div class="bg-white rounded-2xl shadow-soft p-6 border border-surface-container flex flex-col items-center text-center justify-between min-h-[220px] hover:border-primary-container/30 transition-all duration-300">
                <div class="w-16 h-16 bg-[#f0fdf4] rounded-full flex items-center justify-center mb-4">
                    <span class="material-symbols-outlined text-primary-container filled-icon text-3xl">qr_code_scanner</span>
                </div>
                <div class="mb-4">
                    <h4 class="font-bold text-[#171c1f] mb-1">Scan Presensi</h4>
                    <p class="text-xs text-secondary px-2">Scan QR Code yang diberikan guru untuk mencatat kehadiranmu.</p>
                </div>
                <a href="{{ route('presensi.scan') }}"
                   class="w-full bg-primary text-white py-2.5 rounded-xl text-sm font-semibold hover:bg-primary-container transition-colors text-center shadow-soft">
                    Buka Scanner
                </a>
            </div>

            {{-- Materi AI --}}
            <div class="bg-white rounded-2xl shadow-soft p-6 border border-surface-container flex flex-col justify-between min-h-[220px] hover:border-primary-container/30 transition-all duration-300">
                <div>
                    ...
                    <p class="text-xs text-secondary">Akses materi pelajaran beserta ringkasan otomatis dari AI Claude.</p>
                </div>
        ...
        {{-- AI Info --}}
        <div class="bg-[#f0fdf4] border border-primary-container/20 rounded-2xl p-5 flex items-center gap-4 relative overflow-hidden">
            <div class="absolute right-0 top-0 p-4 opacity-5 pointer-events-none">
                <span class="material-symbols-outlined text-[60px] text-primary">auto_awesome</span>
            </div>
            <span class="material-symbols-outlined text-primary-container filled-icon text-2xl shrink-0">auto_awesome</span>
            <p class="text-xs text-on-surface-variant relative z-10 leading-relaxed">
                <span class="font-bold text-primary">Tips Belajar AI:</span>
