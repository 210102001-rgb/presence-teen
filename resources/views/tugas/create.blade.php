<x-app-layout>
    <x-slot name="header">Buat Tugas Baru</x-slot>
    ...
    <div class="p-8">
        <div class="max-w-2xl mx-auto">
            ...
            <nav class="flex items-center gap-1.5 text-xs text-[#5c5f61] mb-6">
                ...
                <span class="text-[#171c1f] font-medium">Buat Baru</span>
            </nav>
            ...
            <div class="bg-white rounded-xl shadow-soft border border-[#eaeef2] p-8">
                <div class="flex items-center gap-3 mb-6 pb-5 border-b border-[#eaeef2]">
                    <div class="w-10 h-10 bg-[#f0fdf4] rounded-xl flex items-center justify-center">
                        <span class="material-symbols-outlined text-[#0e7a3d] filled-icon">assignment_add</span>
                    </div>
                    <div>
                        <h3 class="font-semibold text-[#171c1f]">Tugas Baru</h3>
                        <p class="text-xs text-[#5c5f61]">Isi detail tugas yang akan diberikan kepada siswa</p>
                    </div>
                </div>
                ...
                <form action="{{ route('tugas.store') }}" method="POST" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input id="judul" name="judul" type="text"
                               value="{{ old('judul') }}"
                               placeholder="Contoh: Latihan Soal Bab 4 — Integral"
                    ...
                    <div>
                        ...
                        <textarea id="deskripsi" name="deskripsi" rows="4"
                                  placeholder="Tuliskan petunjuk pengerjaan atau detail tugas..."
                                  class="w-full px-4 py-3 border border-[#becabc] rounded-xl text-sm text-[#171c1f]
                                         focus:outline-none focus:ring-2 focus:ring-[#005f2d] focus:border-[#005f2d] transition-all resize-none">{{ old('deskripsi') }}</textarea>
                        @error('deskripsi')
                            <p class="mt-1 text-xs text-[#ba1a1a]">{{ $message }}</p>
                        @enderror
                    </div>
